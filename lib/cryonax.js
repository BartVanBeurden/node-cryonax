...
// create view scope
utils.createViewScope = function(docs, views) {
	return {
		console: console,
...
// create script scope
utils.createScriptScope = function(docs, views, outputPath) {
	var scope = utils.createViewScope(docs, views);
	scope.save = function(filePath, fileContent) {
		filePath = path.join(outputPath, filePath);
		fileContent = fileContent.replace(/\s+/g, " "); // trim spaces
...
cryonax.build = function(inputPath, outputPath, config) {
	...
	// load docs, views
	var createScope = function() { return utils.createViewScope(docs, vws); }
	docs.load();
	...
	script.run(utils.createScriptScope(docs, vws, outputPath));
	fs.copySync(path.join(inputPath, "static"), outputPath);
};
